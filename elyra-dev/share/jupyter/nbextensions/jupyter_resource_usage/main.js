define([
  ...
  function setupDOM() {
    $('#maintoolbar-container').append(
      $('<div>')
        .attr('id', 'jupyter-resource-usage-display-disk')
        .addClass('btn-group')
        .addClass('jupyter-resource-usage-hide')
        .addClass('pull-right')
        .append($('<strong>').text(' Disk: '))
        .append(
          $('<span>')
            .attr('id', 'jupyter-resource-usage-disk')
            .attr('title', 'Actively used CPU (updates every 5s)')
        )
    );
    $('#maintoolbar-container').append(
      $('<div>')
        .attr('id', 'jupyter-resource-usage-display')
        .addClass('btn-group')
        .addClass('pull-right')
        .append($('<strong>').text('Memory: '))
        .append(
          $('<span>')
            .attr('id', 'jupyter-resource-usage-mem')
            .attr('title', 'Actively used Memory (updates every 5s)')
        )
    );
    $('#maintoolbar-container').append(
      $('<div>')
        .attr('id', 'jupyter-resource-usage-display-cpu')
        .addClass('btn-group')
        .addClass('jupyter-resource-usage-hide')
        .addClass('pull-right')
        ...
        .append(
          $('<span>')
            .attr('id', 'jupyter-resource-usage-cpu')
            .attr('title', 'Actively used CPU (updates every 5s)')
        )
    );
        ...
        success: function (data) {
          ...
          if (limits['memory']) {
            ...
            if (limits['memory']['warn']) {
              $('#jupyter-resource-usage-display').addClass(
                'jupyter-resource-usage-warn'
              );
            } else {
              $('#jupyter-resource-usage-display').removeClass(
                'jupyter-resource-usage-warn'
              );
            }
          }
          ...
          if (cpuPercent !== undefined) {
            ...
            $('#jupyter-resource-usage-display-cpu').removeClass(
              'jupyter-resource-usage-hide'
            );
            display = '';
            ...
            if (limits['cpu']) {
              if (limits['cpu']['warn']) {
                $('#jupyter-resource-usage-display-cpu').addClass(
                  'jupyter-resource-usage-warn'
                );
              } else {
                $('#jupyter-resource-usage-display-cpu').removeClass(
          ...
          if (maxDisk !== undefined) {
            // Remove hide CSS class if the metrics API gives us a CPU percent to display
            $('#jupyter-resource-usage-display-disk').removeClass(
              'jupyter-resource-usage-hide'
            );
            display = '';
            var currentDisk = data['disk_used'];
            display =
              humanFileSize(maxDisk) + ' / ' + humanFileSize(currentDisk);
            // Handle limit warning
            if (limits['disk']) {
              if (limits['disk']['warn']) {
                $('#jupyter-resource-usage-display-disk').addClass(
                  'jupyter-resource-usage-warn'
                );
              } else {
                $('#jupyter-resource-usage-display-disk').removeClass(
