    ...
    56060: (e, a, n) => {
      ...
      const i = {
        ...
        token: function (e, a) {
          if (e.sol()) {
            a.lineNumber++;
            a.inKeywordLine = false;
            if (a.inMultilineTable) {
              a.tableHeaderLine = false;
              if (!e.match(/\s*\|/, false)) {
                a.allowMultilineArgument = false;
                a.inMultilineTable = false;
              }
            }
          }
          e.eatSpace();
          if (a.allowMultilineArgument) {
            if (a.inMultilineString) {
              if (e.match('"""')) {
                a.inMultilineString = false;
                a.allowMultilineArgument = false;
              } else {
                e.match(/.*/);
              }
            ...
            if (a.inMultilineTable) {
              if (e.match(/\|\s*/)) {
                ...
                e.match(/[^\|]*/);
                return a.tableHeaderLine ? 'header' : 'string';
              }
            }
            if (e.match('"""')) {
              a.inMultilineString = true;
              return 'string';
            } else if (e.match('|')) {
              a.inMultilineTable = true;
              a.tableHeaderLine = true;
              return 'bracket';
            }
          }
          if (e.match(/#.*/)) {
            return 'comment';
          } else if (!a.inKeywordLine && e.match(/@\S+/)) {
            return 'tag';
          } else if (
            !a.inKeywordLine &&
            a.allowFeature &&
            ...
            a.allowScenario = true;
            a.allowBackground = true;
            a.allowPlaceholders = false;
            a.allowSteps = false;
            a.allowMultilineArgument = false;
            a.inKeywordLine = true;
            return 'keyword';
          } else if (
            !a.inKeywordLine &&
            a.allowBackground &&
            ...
            return 'string';
          } else if (a.allowPlaceholders && e.match(/<[^>]*>?/)) {
            return 'variable';
          } else {
            e.next();
            e.eatWhile(/[^@"<#]/);
