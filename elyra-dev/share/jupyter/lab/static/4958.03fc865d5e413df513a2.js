    ...
    /***/ 44958: /***/ (
      ...
      __webpack_require__.d(__webpack_exports__, {
        ...
        highlightSelectionMatches: () =>
          /* binding */ highlightSelectionMatches,
      ...
      var index_js_ = __webpack_require__(22819);
      ...
      var dist_index_js_ = __webpack_require__(71674); // CONCATENATED MODULE: ./node_modules/crelt/index.es.js
      function crelt() {
        var elt = arguments[0];
    ...
    */
        nextOverlapping() {
          for (;;) {
            ...
            let str = (0, dist_index_js_.fromCodePoint)(next),
    ...
    */
      ...
      if (typeof Symbol != 'undefined') {
        RegExpCursor.prototype[Symbol.iterator] =
          MultilineRegExpCursor.prototype[Symbol.iterator] = function () {
            return this;
          };
      }
      ...
      function createLineDialog(view) {
        ...
        function go() {
          ...
          let selection = dist_index_js_.EditorSelection.cursor(
            docLine.from + Math.max(0, Math.min(col, docLine.length))
          );
          view.dispatch({
            effects: [
              dialogEffect.of(false),
              index_js_.EditorView.scrollIntoView(selection.from, {
                y: 'center'
              })
            ],
            selection
      ...
      const dialogField = /*@__PURE__*/ dist_index_js_.StateField.define({
        ...
        provide: (f) =>
          index_js_.showPanel.from(f, (val) => (val ? createLineDialog : null))
      });
      /**
...
*/
      const gotoLine = (view) => {
        ...
        if (!panel) {
          ...
          if (view.state.field(dialogField, false) == null)
            effects.push(
              dist_index_js_.StateEffect.appendConfig.of([
                dialogField,
                baseTheme$1
              ])
            );
      ...
      const highlightConfig = /*@__PURE__*/ dist_index_js_.Facet.define({
        combine(options) {
          return (0, dist_index_js_.combineConfig)(
            options,
...
*/
      ...
      const matchDeco = /*@__PURE__*/ index_js_.Decoration.mark({
        class: 'cm-selectionMatch'
      });
      const mainMatchDeco = /*@__PURE__*/ index_js_.Decoration.mark({
        ...
        return (
          (from == 0 ||
            check(state.sliceDoc(from - 1, from)) !=
              dist_index_js_.CharCategory.Word) &&
          (to == state.doc.length ||
      ...
      const matchHighlighter = /*@__PURE__*/ index_js_.ViewPlugin.fromClass(
        class {
          constructor(view) {
          ...
          getDeco(view) {
            ...
            let { state } = view,
              sel = state.selection;
            if (sel.ranges.length > 1) return index_js_.Decoration.none;
            let range = sel.main,
              query,
              check = null;
            if (range.empty) {
              ...
              if (!word) return index_js_.Decoration.none;
              ...
              if (conf.wholeWords) {
                ...
                if (!query) return index_js_.Decoration.none;
              }
            }
            let deco = [];
      ...
      const selectWord = ({ state, dispatch }) => {
        let { selection } = state;
        let newSel = dist_index_js_.EditorSelection.create(
          selection.ranges.map(
            (range) =>
              state.wordAt(range.head) ||
              dist_index_js_.EditorSelection.cursor(range.head)
          ),
          selection.mainIndex
        );
        ...
        dispatch(
          state.update({
            selection: state.selection.addRange(
              dist_index_js_.EditorSelection.range(range.from, range.to),
              false
            ),
...
*/
    ...
    */
        getCursor(state, from = 0, to) {
          let st = state.doc
            ? state
            : dist_index_js_.EditorState.create({ doc: state });
          ...
          return (
            (categorizer(charBefore(buf, from - bufPos)) !=
              ...
              categorizer(charAfter(buf, from - bufPos)) !=
                dist_index_js_.CharCategory.Word) &&
            (categorizer(charAfter(buf, to - bufPos)) !=
              dist_index_js_.CharCategory.Word ||
              categorizer(charBefore(buf, to - bufPos)) !=
                dist_index_js_.CharCategory.Word)
          );
        };
      }
      ...
      function charBefore(str, index) {
        return str.slice(
          (0, dist_index_js_.findClusterBreak)(str, index, false),
          index
        );
      }
      function charAfter(str, index) {
        return str.slice(
          index,
          (0, dist_index_js_.findClusterBreak)(str, index)
        );
      }
      ...
      class RegExpQuery extends QueryType {
        ...
        prevMatchInRange(state, from, to) {
          for (let size = 1; ; size++) {
            let start = Math.max(
              from,
              to - size * 10000 /* FindPrev.ChunkSize */
            );
...
*/
      ...
      const matchMark = /*@__PURE__*/ index_js_.Decoration.mark({
          class: 'cm-searchMatch'
        }),
        selectedMatchMark = /*@__PURE__*/ index_js_.Decoration.mark({
          class: 'cm-searchMatch cm-searchMatch-selected'
        });
      const searchHighlighter = /*@__PURE__*/ index_js_.ViewPlugin.fromClass(
        class {
          constructor(view) {
            this.view = view;
          ...
          highlight({ query, panel }) {
            if (!panel || !query.spec.valid) return index_js_.Decoration.none;
            let { view } = this;
...
*/
      const findNext = /*@__PURE__*/ searchCommand((view, { query }) => {
        ...
        let selection = dist_index_js_.EditorSelection.single(
          next.from,
          next.to
        );
...
*/
      const selectMatches = /*@__PURE__*/ searchCommand((view, { query }) => {
        ...
        view.dispatch({
          selection: dist_index_js_.EditorSelection.create(
            ranges.map((r) =>
              dist_index_js_.EditorSelection.range(r.from, r.to)
            )
          ),
...
*/
      const selectSelectionMatches = ({ state, dispatch }) => {
        ...
        for (
          ...
          ranges.push(
            dist_index_js_.EditorSelection.range(cur.value.from, cur.value.to)
          );
        }
        dispatch(
          state.update({
...
*/
      const replaceNext = /*@__PURE__*/ searchCommand((view, { query }) => {
        ...
        if (next.from == from && next.to == to) {
          ...
          effects.push(
            index_js_.EditorView.announce.of(
              state.phrase(
        ...
        if (next) {
          let off =
            changes.length == 0 || changes[0].from >= match.to
              ? 0
              : match.to - match.from - replacement.length;
          selection = dist_index_js_.EditorSelection.single(
            next.from - off,
            next.to - off
          );
          ...
          view.dispatch({
            effects: [
              ...
              state
                ...
                : dist_index_js_.StateEffect.appendConfig.of(searchExtensions)
            ]
          });
        }
        return true;
...
*/
      ...
      function announceMatch(view, { from, to }) {
        ...
        return index_js_.EditorView.announce.of(
...
//# sourceMappingURL=4958.03fc865d5e413df513a2.js.map?v=03fc865d5e413df513a2
