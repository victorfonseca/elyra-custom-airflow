    ...
    54878: (e, t, n) => {
      ...
      function s(e, t) {
        var n,
          i,
          a = false;
        while (!e.eol() && (n = e.next()) != t.pending) {
          if (n === '$' && i != '\\' && t.pending == '"') {
            a = true;
            break;
          }
      ...
      function o(e, t) {
        var n = e.match(/[\w]+/, false);
        var r = e.match(/(\s+)?\w+\s+=>.*/, false);
        var o = e.match(/(\s+)?[\w:_]+(\s+)?{/, false);
        var c = e.match(/(\s+)?[@]{1,2}[\w:_]+(\s+)?{/, false);
        var u = e.next();
        if (u === '$') {
          if (e.match(a)) {
            return t.continueString ? 'variableName.special' : 'variable';
          }
          return 'error';
        }
        if (t.continueString) {
        ...
        if (n && i.hasOwnProperty(n)) {
          e.backUp(1);
          e.match(/[\w]+/);
          if (e.match(/\s+\S+\s+{/, false)) {
            t.inDefinition = true;
          }
          if (n == 'include') {
            t.inInclude = true;
