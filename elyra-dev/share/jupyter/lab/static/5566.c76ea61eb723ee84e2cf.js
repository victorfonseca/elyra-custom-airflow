...
(self['webpackChunk_jupyterlab_application_top'] =
  ...
  [5566],
  {
    95566: (e, t, r) => {
      ...
      function a(e) {
        var t = e.client || {},
          r = e.atoms || { false: true, true: true, null: true },
          a = e.builtin || c(u),
          i = e.keywords || c(l),
          n = e.operatorChars || /^[*+\-%<>!=&|~^\/]/,
          s = e.support || {},
          o = e.hooks || {},
          d = e.dateSQL || { date: true, time: true, timestamp: true },
          m = e.backslashStringEscapes !== false,
          p = e.brackets || /^[\{}\(\)\[\]]/,
          g = e.punctuation || /^[;.,:]/;
        function h(e, l) {
          var c = e.next();
          ...
          if (
            s.hexNumber &&
            ((c == '0' && e.match(/^[xX][0-9a-fA-F]+/)) ||
              ((c == 'x' || c == 'X') && e.match(/^'[0-9a-fA-F]*'/)))
          ) {
            return 'number';
          } else if (
            s.binaryNumber &&
            ...
            s.decimallessFloat && e.match(/^\.(?!\.)/);
            return 'number';
          } else if (c == '?' && (e.eatSpace() || e.eol() || e.eat(';'))) {
            return 'macroName';
          } else if (c == "'" || (c == '"' && s.doubleQuote)) {
            l.tokenize = b(c);
            return l.tokenize(e, l);
          } else if (
            ((s.nCharCast && (c == 'n' || c == 'N')) ||
              (s.charsetCast && c == '_' && e.match(/[a-z][a-z0-9]*/i))) &&
            (e.peek() == "'" || e.peek() == '"')
          ) {
            return 'keyword';
          } else if (
            s.escapeConstant &&
            (c == 'e' || c == 'E') &&
            (e.peek() == "'" || (e.peek() == '"' && s.doubleQuote))
          ) {
            l.tokenize = function (e, t) {
              return (t.tokenize = b(e.next(), true))(e, t);
            };
            return 'keyword';
          } else if (s.commentSlashSlash && c == '/' && e.eat('/')) {
            e.skipToEnd();
            return 'comment';
          } else if (
            (s.commentHash && c == '#') ||
            (c == '-' && e.eat('-') && (!s.commentSpaceRequired || e.eat(' ')))
          ) {
            e.skipToEnd();
            return 'comment';
          } else if (c == '/' && e.eat('*')) {
            ...
            if (s.zerolessFloat && e.match(/^(?:\d+(?:e[+-]?\d+)?)/i))
              return 'number';
            if (e.match(/^\.+/)) return null;
            if (s.ODBCdotTable && e.match(/^[\w\d_$#]+/)) return 'type';
          } else if (n.test(c)) {
            e.eatWhile(n);
            return 'operator';
          } else if (p.test(c)) {
            ...
            if (
              ...
              return 'number';
            if (r.hasOwnProperty(m)) return 'atom';
            if (a.hasOwnProperty(m)) return 'type';
            if (i.hasOwnProperty(m)) return 'keyword';
            if (t.hasOwnProperty(m)) return 'builtin';
            return null;
          }
        }
        function b(e, t) {
        ...
        function f(e) {
          return function (t, r) {
            var a = t.match(/^.*?(\/\*|\*\/)/);
            if (!a) t.skipToEnd();
            else if (a[1] == '/*') r.tokenize = f(e + 1);
            else if (e > 1) r.tokenize = f(e - 1);
            else r.tokenize = h;
            return 'comment';
          };
        }
        ...
        return {
          ...
          token: function (e, t) {
            ...
            else if (t.context && t.context.type == a) y(t);
            return r;
          },
          indent: function (e, t, r) {
            ...
            if (a.align) return a.col + (i ? 0 : 1);
            else return a.indent + (i ? 0 : r.unit);
          },
          languageData: {
            commentTokens: {
              line: s.commentSlashSlash ? '//' : s.commentHash ? '#' : '--',
              block: { open: '/*', close: '*/' }
            },
      ...
      function i(e) {
        ...
        return e.eatWhile(/\w/) ? 'string.special' : null;
      }
      function n(e) {
        var t;
        while ((t = e.next()) != null) {
      ...
      function o(e) {
        ...
        return e.match(/^[a-zA-Z.#!?]/) ? 'string.special' : null;
      }
      var l =
      ...
      const d = a({
        keywords: c(l + 'begin'),
        builtin: c(u),
        atoms: c('false true null unknown'),
        dateSQL: c('date time timestamp'),
        support: c('ODBCdotTable doubleQuote binaryNumber hexNumber')
      });
      const m = a({
        client: c(
        ...
        dateSQL: c('date datetimeoffset datetime2 smalldatetime datetime time'),
        hooks: { '@': s }
      });
      const p = a({
        client: c(
      ...
      const h = a({
        ...
        atoms: c('null current_date current_time current_timestamp'),
        operatorChars: /^[*+\-%<>!=&|/~]/,
        dateSQL: c('date time timestamp datetime'),
        support: c('decimallessFloat zerolessFloat'),
        identifierQuote: '"',
        hooks: { '@': s, ':': s, '?': s, $: s, '"': n, '`': i }
      });
      const b = a({
        ...
        atoms: c('false true infinity NaN'),
        operatorChars: /^[<>=]/,
        dateSQL: {},
        support: c('commentSlashSlash decimallessFloat'),
        hooks: {}
      });
      const f = a({
        client: c(
        ...
        operatorChars: /^[*\/+\-%<>!=~]/,
        dateSQL: c('date time timestamp'),
        support: c('doubleQuote nCharCast zerolessFloat binaryNumber hexNumber')
      });
      const _ = a({
        keywords: c(
      ...
      const w = a({
        ...
        atoms: c('false true null'),
        operatorChars: /^[*\/+\-%<>!=~&|^]/,
        dateSQL: c('date time timestamp'),
        support: c('ODBCdotTable doubleQuote zerolessFloat')
      });
      const k = a({
        client: c('source'),
        keywords: c(
