  ...
  [1268],
  {
    /***/ 91268: /***/ (
      ...
      /* harmony export */ __webpack_require__.d(__webpack_exports__, {
        /* harmony export */ KO: () => /* binding */ buildParser
        /* harmony export */
      });
      /* unused harmony exports GenError, buildParserFile */
      /* harmony import */ var _lezer_common__WEBPACK_IMPORTED_MODULE_0__ =
        __webpack_require__(66575);
      /* harmony import */ var _lezer_common__WEBPACK_IMPORTED_MODULE_0___default =
        /*#__PURE__*/ __webpack_require__.n(
          _lezer_common__WEBPACK_IMPORTED_MODULE_0__
        );
      /* harmony import */ var _lezer_lr__WEBPACK_IMPORTED_MODULE_1__ =
        __webpack_require__(27421);
      ...
      class NameExpression extends Expression {
        ...
        walk(f) {
          ...
          return f(
            args == this.args
              ? this
              : new NameExpression(this.start, this.id, args)
          );
        }
      }
      ...
      class ChoiceExpression extends Expression {
        ...
        walk(f) {
          ...
          return f(
            exprs == this.exprs ? this : new ChoiceExpression(this.start, exprs)
          );
        }
      }
      ...
      class SequenceExpression extends Expression {
        ...
        toString() {
          return this.empty
            ? '()'
            : this.exprs.map((e) => maybeParens(e, this)).join(' ');
        }
        eq(other) {
          return (
      ...
      class RepeatExpression extends Expression {
        ...
        walk(f) {
          ...
          return f(
            expr == this.expr
              ? this
              : new RepeatExpression(this.start, expr, this.kind)
          );
        }
      }
      ...
      class Prop extends Node {
        ...
        eq(other) {
          return (
            ...
            this.value.every(
              (v, i) =>
                v.value == other.value[i].value && v.name == other.value[i].name
            )
          );
        }
        toString() {
...
*/
      ...
      class Term {
        ...
        get terminal() {
          return (this.flags & 1) /* TermFlag.Terminal */ > 0;
        }
        get eof() {
          return (this.flags & 4) /* TermFlag.Eof */ > 0;
        }
        get error() {
          return 'error' in this.props;
        }
        get top() {
          return (this.flags & 2) /* TermFlag.Top */ > 0;
        }
        get interesting() {
          return this.flags > 0 || this.nodeName != null;
        }
        get repeated() {
          return (this.flags & 16) /* TermFlag.Repeated */ > 0;
        }
        set preserve(value) {
          this.flags = value
            ? this.flags | 8 /* TermFlag.Preserve */
            : this.flags & ~8 /* TermFlag.Preserve */;
        }
        get preserve() {
          return (this.flags & 8) /* TermFlag.Preserve */ > 0;
        }
        set inline(value) {
          this.flags = value
            ? this.flags | 32 /* TermFlag.Inline */
            : this.flags & ~32 /* TermFlag.Inline */;
        }
        get inline() {
          return (this.flags & 32) /* TermFlag.Inline */ > 0;
        }
        cmp(other) {
      ...
      class TermSet {
        constructor() {
          ...
          this.eof = this.term(
            '␄',
            null,
            1 /* TermFlag.Terminal */ | 4 /* TermFlag.Eof */
          );
        ...
        finish(rules) {
          ...
          this.terms = this.terms.filter(
            (t) =>
              t.terminal ||
              t.preserve ||
              rules.some((r) => r.name == t || r.parts.includes(t))
          );
          let names = {};
      ...
      class Conflicts {
        ...
        cmp(other) {
          return (
            ...
            cmpSet(this.ambigGroups, other.ambigGroups, (a, b) =>
              a < b ? -1 : a > b ? 1 : 0
            ) ||
            this.cut - other.cut
          );
      ...
      function minimize(states, start) {
        ...
        for (;;) {
          let split = false,
            newPartition = Object.create(null);
          for (let state of states) {
      ...
      function isEquivalent(a, b, partition) {
        ...
        for (let i = 0; i < a.edges.length; i++) {
          ...
          if (
            eA.from != eB.from ||
            eA.to != eB.to ||
            partition[eA.target.id] != partition[eB.target.id]
          )
            return false;
        }
        return true;
      ...
      let State$1 = class State {
        ...
        compile() {
          ...
          function explore(states) {
            let newState = (labeled[ids(states)] = new State(
              states.reduce((a, s) => union(a, s.accepting), []),
              localID++
            ));
            let out = [];
            for (let state of states)
            ...
            for (let merged of transitions) {
              ...
              newState.edge(
                merged.from,
                merged.to,
                labeled[ids(targets)] || explore(targets)
              );
            }
            return newState;
          }
          ...
          function explore(state) {
            ...
            if (
              ...
              (state.accepting.length > 0 &&
                !state.edges.some((e) =>
                  sameSet$1(state.accepting, e.target.accepting)
                ))
            )
              result.push(state);
            for (let edge of state.edges)
            ...
            if (!found)
              conflicts.push(
                new Conflict$1(
                  ...
                  b,
                  soft,
                  exampleFromEdges(aEdges),
                  bEdges && exampleFromEdges(bEdges)
                )
              );
                  ...
                  for (let orig of state.accepting)
                    if (term != orig)
                      add(
                        ...
                        orig,
                        hasCycle ||
                          cycleTerms.includes(orig) ||
                          !occurTogether(term, orig)
                          ? 0
                          : 1,
                        edges,
                        edges.concat(es)
        ...
        toArray(groupMasks, precedence) {
          ...
          this.reachable((state) => {
            ...
            data.push(
              state.stateMask(groupMasks),
              acceptEnd,
              state.edges.length
            );
            state.accepting.sort(
              (a, b) => precedence.indexOf(a.id) - precedence.indexOf(b.id)
            );
            for (let term of state.accepting)
      ...
      function mergeEdges(edges) {
        ...
        let eof = edges.filter(
          (e) => e.from == 65535 /* Seq.End */ && e.to == 65535 /* Seq.End */
        );
        if (eof.length) {
          let found = [];
          for (let edge of eof)
          ...
          if (found.length)
            result.push(
              new MergedEdge(65535 /* Seq.End */, 65535 /* Seq.End */, found)
            );
        }
        return result;
      }
      ...
      class Input {
        ...
        next() {
          ...
          if (next == '"') {
            ...
            return this.set(
              'string',
              readString(this.string.slice(start + 1, end - 1)),
              start,
              end
            );
          } else if (next == "'") {
            ...
            return this.set(
              'set',
              this.string.slice(start + 2, end - 1),
              start,
              end
            );
          } else if (/[\[\]()!~+*?{}<>\.,|:$=]/.test(next)) {
            ...
            if (!m)
              return this.raise(
                'Unexpected character ' + JSON.stringify(next),
                start
              );
        ...
        while (input.type != 'eof') {
          ...
          if (input.eat('at', 'top')) {
            ...
            if (input.eat('id', 'tokens'))
              ...
              specialized.push(
                parseExternalSpecialize(input, 'specialize', start)
              );
            ...
            if (input.type == '{') {
              ...
              if (mainSkip)
                input.raise(
                  `Multiple top-level skip declarations`,
                  input.start
                );
              mainSkip = skip;
            }
      ...
      function parseExprInner(input) {
        ...
        if (input.eat('(')) {
          ...
          input.type == 'at' &&
          (input.value == 'specialize' || input.value == 'extend')
        ) {
          ...
          input.type == 'at' &&
          CharClasses.hasOwnProperty(input.value)
        ) {
      ...
      function parseExprChoice(input) {
        ...
        let empty = exprs.find(
          (s) => s instanceof SequenceExpression && s.empty
        );
        if (empty)
          input.raise(
      ...
      function parseLocalTokens(input, start) {
        ...
        return new LocalTokenDeclaration(
          start,
          precedences,
          tokenRules,
          fallback
      ...
      function parseExternalTokens(input, start) {
        ...
        return new ExternalTokenDeclaration(
          start,
          id,
          from,
          parseExternalTokenSet(input)
        );
      }
      ...
      function parseExternalPropSource(input, start) {
        let id = parseIdent(input);
        input.expect('id', 'from');
        return new ExternalPropSourceDeclaration(
          start,
          id,
          input.expect('string')
        );
      ...
      class Shift {
        ...
        matches(other, mapping) {
          return (
            other instanceof Shift &&
            mapping[other.target.id] == mapping[this.target.id]
          );
        }
        toString() {
      ...
      class Reduce {
        ...
        eq(other) {
          return (
            ...
            this.term == other.term &&
            other.rule.sameReduce(this.rule)
          );
        }
        cmp(other) {
      ...
      class State {
          ...
          check: for (let i = 0; i < this.actions.length; i++) {
            ...
            if (action.term == value.term) {
              ...
              if (diff > 0) {
                ...
                conflicts.ambigGroups.some((g) =>
                  actionConflicts.ambigGroups.includes(g)
                )
              ) {
        ...
        addAction(value, positions, context) {
          ...
          if (conflict) {
            let conflictPos =
              this.actionPositions[this.actions.indexOf(conflict)][0];
            let rules = [positions[0].rule.name, conflictPos.rule.name];
            if (
              context.conflicts.some((c) =>
                c.rules.some((r) => rules.includes(r))
              )
            )
              return;
            let error;
        ...
        actionsByTerm() {
          ...
          if (!result) {
            ...
            for (let action of this.actions)
              (result[action.term.id] || (result[action.term.id] = [])).push(
                action
              );
          }
          return result;
        ...
        finish() {
          if (this.actions.length) {
            ...
            if (first instanceof Reduce) {
              ...
              if (
                this.actions.every(
                  (a) => a instanceof Reduce && a.rule.sameReduce(rule)
                )
              )
      ...
      function closure(set, first) {
        ...
        for (let pos of set) {
          ...
          if (next && !next.terminal)
            addFor(
              ...
              pos.pos == pos.rule.parts.length - 1
                ? pos.skipAhead
                : pos.rule.skip,
              pos
            );
        ...
        while (redo.length) {
          ...
          addFor(
            ...
            union(
              ...
              add.rule.parts.length == 1 ? add.ambigAhead : none$1
            ),
        ...
        for (let add of added) {
          ...
          add.finish();
          let origIndex = set.findIndex(
            (p) => p.pos == 0 && p.rule == add.rule
          );
        ...
        function explore(pos, i, hasMatch) {
          ...
          for (let next; (next = pos.next); ) {
            ...
            if (
              ...
              (after.terminal
                ? after == termAfter
                : first[after.name].includes(termAfter))
            )
              match = pos.advance();
      ...
      function buildFullAutomaton(terms, startTerms, first) {
        ...
        function getState(core, top) {
            ...
            for (let known of byHash)
              if (eqSet(core, known.set)) {
                if (known.state.skip != skip)
                  throw new GenError(
                    'Inconsistent skip sets after ' + known.set[0].trail()
                  );
                return known.state;
              }
          ...
          let found;
          if (!top)
            for (let state of forHash) if (state.hasSet(set)) found = state;
          if (!found) {
            ...
            if (timing && states.length % 500 == 0)
              console.log(
                `${states.length} states after ${((Date.now() - t0) / 1000).toFixed(2)}s`
              );
          }
          (cores[coreHash] || (cores[coreHash] = [])).push(
            new Core(core, found)
          );
          return found;
        }
        for (const startTerm of startTerms) {
          const startSkip = startTerm.rules.length
            ? startTerm.rules[0].skip
            : terms.names['%noskip'];
          getState(
            startTerm.rules.map((rule) =>
              new Pos(rule, 0, [terms.eof], none$1, startSkip, null).finish()
            ),
            startTerm
          );
        }
          ...
          if (replaced)
            for (let i = 0; i < state.goto.length; i++) {
              ...
              if (
                !start.some((term) =>
                  state.actions.some(
                    (a) => a.term == term && a instanceof Shift
                  )
                )
              )
        ...
        if (conflicts.conflicts.length)
          throw new GenError(
            conflicts.conflicts.map((c) => c.error).join('\n\n')
          );
      ...
      function canMerge(a, b, mapping) {
        ...
        for (let goto of a.goto)
          for (let other of b.goto) {
            if (
              goto.term == other.term &&
              mapping[goto.target.id] != mapping[other.target.id]
            )
              return false;
          }
        ...
        function spill(groupIndex, index) {
          ...
          for (let i = groupIndex + 1; i < groups.length; i++) {
            ...
            if (
              ...
              groups[i].members.every((id) =>
                canMerge(state, states[id], mapping)
              )
            ) {
      ...
      class Parts {
        ...
        concat(other) {
          ...
          if (this.conflicts || other.conflicts) {
            ...
            conflicts[conflicts.length - 1] = conflicts[
              conflicts.length - 1
            ].join(otherConflicts[0]);
      ...
      class Builder {
        constructor(text, options) {
          ...
          for (let prop in NP) {
            if (
              NP[prop] instanceof
                _lezer_common__WEBPACK_IMPORTED_MODULE_0__.NodeProp &&
              !NP[prop].perNode
            )
              this.knownProps[prop] = {
                prop: NP[prop],
                source: { name: prop, from: null }
              };
          }
          for (let prop of this.ast.externalProps) {
            this.knownProps[prop.id.name] = {
              prop: this.options.externalProp
                ...
                : new _lezer_common__WEBPACK_IMPORTED_MODULE_0__.NodeProp(),
          ...
          this.localTokens = this.ast.localTokens.map(
            (g) => new LocalTokenSet(this, g)
          );
          this.externalTokens = this.ast.externalTokens.map(
            (ext) => new ExternalTokenSet(this, ext)
          );
          ...
          time('Build rules', () => {
            ...
            let mainSkip = this.ast.mainSkip
              ? this.newName('%mainskip', true)
              : noSkip;
            let scopedSkip = [],
              topRules = [];
            ...
            for (let scoped of this.ast.scopedSkip) {
              let skip = noSkip,
                found = this.ast.scopedSkip.findIndex(
                  (sc, i) =>
                    i < scopedSkip.length && exprEq(sc.expr, scoped.expr)
                );
              if (found > -1) skip = scopedSkip[found];
              else if (
                this.ast.mainSkip &&
                exprEq(scoped.expr, this.ast.mainSkip)
              )
                skip = mainSkip;
            ...
            this.skipRules =
              mainSkip == noSkip ? [mainSkip] : [noSkip, mainSkip];
            if (mainSkip != noSkip)
            ...
            for (let i = 0; i < this.ast.scopedSkip.length; i++) {
              ...
              if (!this.skipRules.includes(skip)) {
                ...
                if (skip != noSkip)
                  this.defineRule(
                    skip,
                    this.normalizeExpr(this.ast.scopedSkip[i].expr)
                  );
              }
            }
            this.currentSkip.pop();
            for (let { rule, skip } of topRules.sort(
              (a, b) => a.rule.start - b.rule.start
            )) {
              this.unique(rule.id);
              this.used(rule.id.name);
              this.currentSkip.push(skip);
              let { name, props } = this.nodeInfo(
                rule.props,
                'a',
                rule.id.name,
                none,
            ...
            for (let { skip, rule } of this.astRules) {
              if (
                this.ruleNames[rule.id.name] &&
                isExported(rule) &&
                !rule.params.length
              ) {
                this.buildRule(rule, [], skip, false);
                if (
                  rule.expr instanceof SequenceExpression &&
                  rule.expr.exprs.length == 0
                )
                  this.used(rule.id.name);
              }
            }
        ...
        newName(base, nodeName = null, props = {}) {
          for (let i = nodeName ? 0 : 1; ; i++) {
            ...
            if (!this.terms.names[name])
              return this.terms.makeNonTerminal(
                name,
                nodeName === true ? null : nodeName,
                props
              );
          }
        }
        prepareParser() {
          ...
          let localTokens = this.localTokens.map((grp, i) =>
            grp.buildLocalGroup(fullTable, skipInfo, i)
          );
          let { tokenGroups, tokenPrec, tokenData } = time(
            'Build token groups',
            () =>
              this.tokens.buildTokenGroups(
                fullTable,
                skipInfo,
                localTokens.length
              )
          );
          let table = time('Finish automaton', () =>
            finishAutomaton(fullTable)
          );
          ...
          let states = time('Finish states', () => {
            let states = new Uint32Array(
              table.length * 6 /* ParseState.Size */
            );
          ...
          for (let term of this.terms.tops)
            topRules[term.nodeName] = [
              table.find((state) => state.startRule == term).id,
              term.id
            ];
          let precTable = data.storeArray(
            tokenPrec.concat(65535 /* Seq.End */)
          );
        ...
        getParser() {
          ...
          let specialized = rawSpecialized.map((v) => {
            if (v instanceof ExternalSpecializer) {
              let ext = this.options.externalSpecializer(
                v.ast.id.name,
                this.termTable
              );
              return {
          ...
          return _lezer_lr__WEBPACK_IMPORTED_MODULE_1__ /* .LRParser */.U1.deserialize(
            {
              ...
              maxTerm,
              repeatNodeCount,
              nodeProps: rawNodeProps.map(({ prop, terms }) => [
                this.knownProps[prop].prop,
                ...terms
              ]),
              propSources: !this.options.externalPropSource
                ? undefined
                : this.ast.externalPropSources.map((s) =>
                    this.options.externalPropSource(s.id.name)
                  ),
        ...
        getParserFile() {
          ...
          let nodeProps = rawNodeProps.map(({ prop, terms }) => {
            ...
            let propID = source.from
              ? importName(source.name, source.from)
          ...
          let specialized = rawSpecialized.map((v) => {
            if (v instanceof ExternalSpecializer) {
              ...
              let tableName = getName(
                'spec_' + v.token.name.replace(/\W/g, '')
              );
              defined[tableName] = true;
              specHead += `const ${tableName} = ${specializationTableString(v.table)}\n`;
              let ts = this.options.typeScript
                ? `: keyof typeof ${tableName}`
                : '';
              return `{term: ${v.token.id}, get: (value${ts}) => ${tableName}[value] || -1}`;
            }
          });
          let propSources = this.ast.externalPropSources.map((s) =>
            importName(s.id.name, s.source)
          );
          ...
          function serializePropValue(value) {
            return typeof value != 'string' ||
              /^(true|false|\d+(\.\d+)?|\.\d+)$/.test(value)
              ? value
              : JSON.stringify(value);
          }
          let dialects = Object.keys(rawDialects).map(
            (d) => `${d}: ${rawDialects[d]}`
          );
  ...
  termNames: ${JSON.stringify(termNames)}`
      : ''
          ...
          for (let name in this.termTable) {
            ...
            terms.push(
              `${id}${mod == 'cjs' ? ':' : ' ='} ${this.termTable[name]}`
            );
          }
          for (let id = 0; id < this.dialects.length; id++)
            terms.push(
              `Dialect_${this.dialects[id]}${mod == 'cjs' ? ':' : ' ='} ${id}`
            );
          return {
            parser:
              head +
        ...
        gatherNonSkippedNodes() {
          ...
          for (let i = 0; i < work.length; i++) {
            for (let rule of work[i].rules)
              for (let part of rule.parts) add(part);
          }
          return seen;
        }
          ...
          for (let type of nodeTypes) {
            ...
            for (let prop in type.props) {
              ...
              if (
                ...
                (known.source.name == 'repeated' ||
                  known.source.name == 'error')
              )
                continue;
              let rec = nodeProps.find((r) => r.prop == prop);
              if (!rec) nodeProps.push((rec = { prop, values: {} }));
              (
                rec.values[type.props[prop]] ||
                (rec.values[type.props[prop]] = [])
              ).push(type.id);
            }
          }
        ...
        makeTerminal(name, tag, props) {
          return this.terms.makeTerminal(
            this.terms.uniqueName(name),
            tag,
            props
          );
        }
        computeForceReductions(states, skipInfo) {
          ...
          for (let state of states) {
            ...
            for (let edge of state.goto) {
              let array =
                gotoEdges[edge.term.id] || (gotoEdges[edge.term.id] = []);
            ...
            candidates[state.id] = state.set
              ...
              .sort(
                (a, b) =>
                  b.pos - a.pos || a.rule.parts.length - b.rule.parts.length
              );
          }
          ...
          function createsCycle(term, startState, parents = null) {
            ...
            return edges.some((val) => {
              ...
              return (
                found != null &&
                createsCycle(found, startState, parentIntersection)
              );
            });
          }
          for (let state of states) {
            if (state.defaultReduce && state.defaultReduce.parts.length > 0) {
              reductions[state.id] = reduceAction(
                state.defaultReduce,
                skipInfo
              );
          ...
          let substituteInValue = (value) => {
            ...
            for (let i = 0; i < value.length; i++) {
              ...
              if (expr instanceof NameExpression && !expr.args.length)
                ...
                this.raise(
                  `Trying to interpolate expression '${expr}' into a prop`,
                  part.start
                );
            }
            return result;
        ...
        conflictsFor(markers) {
          ...
          for (let marker of markers) {
            if (marker.type == 'ambig') {
              ...
              let index = precs
                ? precs.items.findIndex(
                    (item) => item.id.name == marker.id.name
                  )
                : -1;
              if (index < 0)
                this.raise(
                  `Reference to unknown precedence: '${marker.id.name}'`,
                  marker.id.start
                );
              ...
              if (prec.type == 'cut') {
                ...
                atEnd = atEnd.join(
                  new Conflicts(
                    (value << 2) +
                      (prec.type == 'left' ? 1 : prec.type == 'right' ? -1 : 0)
                  )
                );
              }
            }
        ...
        defineRule(name, choices) {
          ...
          for (let choice of choices)
            this.rules.push(
              new Rule(name, choice.terms, choice.ensureConflicts(), skip)
            );
        }
        resolve(expr) {
          for (let built of this.built)
            if (built.matches(expr)) return [p(built.term)];
          ...
          if (!known)
            return this.raise(
              `Reference to undefined rule '${expr.id.name}'`,
              expr.start
            );
          if (known.rule.params.length != expr.args.length)
            this.raise(
              `Wrong number or arguments for '${expr.id.name}'`,
              expr.start
            );
        ...
        normalizeSequence(expr) {
          ...
          function complete(start, from, endConflicts) {
            ...
            if (from == result.length)
              return [
                start.withConflicts(start.terms.length, here.join(endConflicts))
              ];
            let choices = [];
        ...
        normalizeExpr(expr) {
          if (expr instanceof RepeatExpression && expr.kind == '?') {
            ...
            return expr.exprs.reduce(
              (o, e) => o.concat(this.normalizeExpr(e)),
              []
            );
        ...
        buildRule(rule, args, skip, inline = false) {
          ...
          if (
            parts.length >
            100 * (expr instanceof ChoiceExpression ? expr.exprs.length : 1)
          )
            this.warn(
        ...
        nodeInfo(
          ...
          let name =
            defaultName &&
            (allow.indexOf('a') > -1 || !ignored(defaultName)) &&
            !/ /.test(defaultName)
              ? defaultName
              : null;
          let dialect = null,
            ...
            if (!prop.at) {
              if (!this.knownProps[prop.name]) {
                let builtin = [
                  ...
                  : '';
                this.raise(
                  `Unknown prop name '${prop.name}'${builtin}`,
                  prop.start
                );
              }
              ...
              if (/ /.test(name))
                this.raise(
                  `Node names cannot have spaces ('${name}')`,
                  prop.start
                );
              ...
              if (dialectID < 0)
                this.raise(
                  `Unknown dialect '${prop.value[0].value}'`,
                  prop.value[0].start
                );
              dialect = dialectID;
              ...
              group = prop.value.length
                ? this.finishProp(prop, args, params)
                : defaultName;
              ...
              this.raise(
                `Unknown built-in prop name '@${prop.name}'`,
                prop.start
              );
            }
          }
          ...
          if (defaultProps && hasProps(defaultProps)) {
            for (let prop in defaultProps)
              if (!(prop in result)) result[prop] = defaultProps[prop];
          }
          if (hasProps(result) && !name)
            this.raise(
              `Node has properties but no name`,
              props.length ? props[0].start : expr.start
            );
        ...
        resolveSpecialization(expr) {
          let type = expr.type;
          let { name, props, dialect, exported } = this.nodeInfo(
            expr.props,
            'd'
          );
          ...
          if (
            terminal.length != 1 ||
            terminal[0].terms.length != 1 ||
            !terminal[0].terms[0].terminal
          )
            this.raise(
              `The first argument to '${type}' must resolve to a token`,
              expr.token.start
            );
          let values;
          if (expr.content instanceof LiteralExpression)
            ...
            expr.content.exprs.every((e) => e instanceof LiteralExpression)
          )
          ...
          let term = terminal[0].terms[0],
            token = null;
          let table =
            this.specialized[term.name] || (this.specialized[term.name] = []);
          for (let value of values) {
            let known = table.find((sp) => sp.value == value);
            if (known == null) {
              if (!token) {
                token = this.makeTerminal(
                  term.name + '/' + JSON.stringify(value),
                  name,
                  props
                );
                if (dialect != null)
                  (
                    this.tokens.byDialect[dialect] ||
                    (this.tokens.byDialect[dialect] = [])
                  ).push(token);
              }
        ...
        findDelimiters(expr) {
          ...
          let findToken = (expr) => {
            ...
            if (expr instanceof NameExpression && expr.args.length == 0) {
              ...
              let token = this.tokens.rules.find(
                (r) => r.id.name == expr.id.name
              );
              if (token && token.expr instanceof LiteralExpression)
                return {
                  term: this.tokens.getToken(expr),
                  str: token.expr.value
                };
            }
            return null;
          };
          ...
          let bracket = brackets.find(
            (b) =>
              lastToken.str.indexOf(b[1]) > -1 &&
              lastToken.str.indexOf(b[0]) < 0
          );
        ...
        defineGroup(rule, group, ast) {
          ...
          for (let name of getNamed(rule))
            name.props['group'] = (
              ((_a = name.props['group']) === null || _a === void 0
                ? void 0
                : _a.split(' ')) || []
            )
              .concat(group)
        ...
        checkGroups() {
          ...
          for (let i = 0; i < names.length; i++) {
            ...
            if (nodeNames[name])
              this.warn(
                `Group name '${name}' conflicts with a node of the same name`
              );
      ...
      class FinishStateContext {
        ...
        findSharedActions(state) {
          ...
          for (let i = state.id + 1; i < this.states.length; i++) {
            ...
            for (let a of state.actions)
              for (let b of other.actions) if (a.eq(b)) scratch[fill++] = a;
        ...
        storeActions(actions, skipReduce, shared) {
          if (
            skipReduce < 0 &&
            shared &&
            shared.actions.length == actions.length
          )
            return shared.addr;
          let data = [];
          ...
          if (skipReduce > -1)
            data.push(
              2 /* Seq.Other */,
              skipReduce & 65535 /* Action.ValueMask */,
              skipReduce >> 16
            );
          else if (shared)
            data.push(
        ...
        finish(state, isSkip, forcedReduce) {
          ...
          let defaultReduce = state.defaultReduce
            ? reduceAction(state.defaultReduce, this.skipInfo)
            : 0;
          ...
          if (
            state.set.some(
              (p) => p.rule.name.top && p.pos == p.rule.parts.length
            )
          )
          ...
          for (let i = 0; i < state.actions.length + skipTerms.length; i++) {
            let term =
              i < state.actions.length
                ? state.actions[i].term
                : skipTerms[i - state.actions.length];
            for (;;) {
          ...
          this.stateArray[base + 1 /* ParseState.Actions */] =
            this.storeActions(
          ...
          this.stateArray[base + 3 /* ParseState.TokenizerMask */] =
            tokenizerMask;
          this.stateArray[base + 4 /* ParseState.DefaultReduce */] =
            defaultReduce;
          this.stateArray[base + 5 /* ParseState.ForcedReduce */] =
            forcedReduce;
        }
      }
      ...
      function reduceAction(rule, skipInfo, depth = rule.parts.length) {
        return (
          ...
          (rule.isRepeatWrap && depth == rule.parts.length
            ? 131072 /* Action.RepeatFlag */
            : 0) |
          (skipInfo.some((i) => i.rule == rule.name)
            ? 262144 /* Action.StayFlag */
            : 0) |
      ...
      function computeGotoTable(states) {
        ...
        for (let state of states) {
          for (let entry of state.goto) {
            ...
            (set[entry.target.id] || (set[entry.target.id] = [])).push(
              state.id
            );
          }
        }
        ...
        for (let term = 0; term <= maxTerm; term++) {
          ...
          for (let target of keys) {
            ...
            termTable.push(
              (target == keys[keys.length - 1] ? 1 : 0) + (list.length << 1)
            );
      ...
      class TokenSet {
        ...
        getToken(expr) {
          ...
          if (dialect != null)
            (this.byDialect[dialect] || (this.byDialect[dialect] = [])).push(
              term
            );
        ...
        buildRule(rule, expr, from, to, args = none) {
          ...
          if (rule.params.length != expr.args.length)
            this.b.raise(
              `Incorrect number of arguments for token '${name}'`,
              expr.start
            );
          let building = this.building.find(
            (b) => b.name == name && exprsEq(expr.args, b.args)
          );
          if (building) {
            if (building.to == to) {
        ...
        build(expr, from, to, args) {
          if (expr instanceof NameExpression) {
            ...
            if (!rule)
              return this.b.raise(
                `Reference to token rule '${expr.id.name}', which isn't found`,
                expr.start
              );
            ...
            if (conflict)
              this.b.raise(
                'Conflict marker in token expression',
                conflict[0].start
              );
          ...
          if (this.ast)
            for (let group of this.ast.precedences) {
              ...
              for (let item of group.items) {
                ...
                if (!level.length)
                  this.b.warn(
                    `Precedence specified for unknown token ${item}`,
                    item.start
                  );
        ...
        buildPrecTable(softConflicts) {
          ...
          add: while (rel.length) {
            ...
            this.b.raise(
              `Cyclic token precedence relation between ${rel.map((r) => r.term).join(', ')}`
            );
          }
          return precTable;
        }
      ...
      class MainTokenSet extends TokenSet {
        ...
        getLiteral(expr) {
          ...
          let name = null,
            ...
            exported = null;
          let decl = this.ast
            ? this.ast.literals.find((l) => l.literal == expr.value)
            : null;
          if (decl)
            ({ name, props, dialect, exported } = this.b.nodeInfo(
              decl.props,
              'da',
              expr.value
            ));
        ...
        takeConflicts() {
          ...
          let resolve = (expr) => {
            ...
            this.b.warn(
              `Precedence specified for unknown token ${expr}`,
              expr.start
            );
            return null;
          };
          for (let c of ((_a = this.ast) === null || _a === void 0
            ? void 0
            : _a.conflicts) || []) {
            let a = resolve(c.a),
              b = resolve(c.b);
        ...
        buildTokenGroups(states, skipInfo, startID) {
          ...
          if (tokens.accepting.length)
            this.b.raise(
              ...
              this.rules.find((r) => r.id.name == tokens.accepting[0].name)
                .start
            );
          ...
          let allConflicts = tokens
            ...
            .filter(
              ({ a, b }) => !this.precededBy(a, b) && !this.precededBy(b, a)
            );
          ...
          for (let state of states) {
            ...
            for (let term of skip)
              if (state.actions.some((a) => a.term == term))
                this.b.raise(
                  `Use of token ${term.name} conflicts with skip rule`
                );
            let stateTerms = [];
            for (
              let i = 0;
              i < state.actions.length + (skip ? skip.length : 0);
              i++
            ) {
              let term =
                i < state.actions.length
                  ? state.actions[i].term
                  : skip[i - state.actions.length];
            ...
            for (let term of stateTerms) {
              for (let conflict of conflicts) {
                ...
                if (
                  stateTerms.includes(conflicting) &&
                  !errors.some((e) => e.conflict == conflict)
                ) {
                  let example = conflict.exampleA
                    ? ` (example: ${JSON.stringify(conflict.exampleA)}${conflict.exampleB ? ` vs ${JSON.stringify(conflict.exampleB)}` : ''})`
                    : '';
                  errors.push({
                    error:
      ...
      class LocalTokenSet extends TokenSet {
        ...
        getToken(expr) {
          ...
          if (this.ast.fallback && this.ast.fallback.id.name == expr.id.name) {
            if (expr.args.length)
              this.b.raise(
                `Incorrect number of arguments for ${expr.id.name}`,
                expr.start
              );
            if (!this.fallback) {
              let {
              ...
              let term = (this.fallback = this.b.makeTerminal(
                expr.id.name,
                nodeName,
                props
              ));
        ...
        buildLocalGroup(states, skipInfo, id) {
          ...
          for (let { a, b, exampleA } of tokens.findConflicts(() => true)) {
            if (!this.precededBy(a, b) && !this.precededBy(b, a))
              this.b.raise(
                `Overlapping tokens ${a.name} and ${b.name} in local token group${exampleA ? ` (example: ${JSON.stringify(exampleA)})` : ''}`
              );
          }
          for (let state of states) {
            if (state.defaultReduce) continue;
            ...
            for (let { term } of state.actions) {
              let orig = this.b.tokenOrigins[term.name];
              if (
                (orig === null || orig === void 0 ? void 0 : orig.group) == this
              )
                usesThis = term;
              else usesOther = term;
            }
            if (usesThis) {
              if (usesOther)
                this.b.raise(
                  `Tokens from a local token group used together with other tokens (${usesThis.name} with ${usesOther.name})`
                );
              state.tokenGroup = id;
            }
          }
          ...
          let fullData = new Uint16Array(
            tokenData.length + precTable.length + 1
          );
          ...
          return {
            groupID: id,
            create: () =>
              new _lezer_lr__WEBPACK_IMPORTED_MODULE_1__ /* .LocalTokenGroup */.uC(
                fullData,
                precOffset,
                this.fallback ? this.fallback.id : undefined
              ),
            createSource: (importName) =>
              `new ${importName('LocalTokenGroup', '@lezer/lr')}(${encodeArray(fullData)}, ${precOffset}${this.fallback ? `, ${this.fallback.id}` : ''})`
          };
        }
      }
      function checkTogether(states, b, skipInfo) {
        ...
        return (a, b) => {
          ...
          return (cache[key] = states.some(
            (state) => hasTerm(state, a) && hasTerm(state, b)
          ));
        };
      }
      ...
      function gatherExtTokens(b, tokens) {
        ...
        for (let token of tokens) {
          b.unique(token.id);
          let { name, props, dialect } = b.nodeInfo(
            token.props,
            'd',
            token.id.name
          );
          ...
          if (dialect != null)
            (
              b.tokens.byDialect[dialect] || (b.tokens.byDialect[dialect] = [])
            ).push(term);
      ...
      class ExternalSpecializer {
        ...
        finish() {
          ...
          for (let name in this.tokens)
            this.b.tokenOrigins[this.tokens[name].name] = {
              spec: this.term,
              external: this
            };
        }
      ...
      function inlineRules(rules, preserve) {
        for (let pass = 0; ; pass++) {
          let inlinable = Object.create(null),
            found;
          if (pass == 0)
          ...
          for (let i = 0; i < rules.length; i++) {
            ...
            if (
              ...
              (rule.parts.length == 1 ||
                rules.every(
                  (other) =>
                    other.skip == rule.skip || !other.parts.includes(rule.name)
                )) &&
          ...
          for (let rule of rules) {
            ...
            function expand(at, conflicts, parts) {
              ...
              if (!replace) {
                expand(
                  at + 1,
                  conflicts.concat(rule.conflicts[at + 1]),
                  parts.concat(next)
                );
                return;
              }
              for (let r of replace)
                expand(
                  ...
                  conflicts
                    ...
                    .concat(
                      rule.conflicts[at + 1].join(
                        r.conflicts[r.conflicts.length - 1]
                      )
                    ),
...
//# sourceMappingURL=1268.ab0c2924a29c0558b00e.js.map?v=ab0c2924a29c0558b00e
