    ...
    /***/ 79311: /***/ (
      ...
      __webpack_require__.d(__webpack_exports__, {
        ...
        insertNewlineContinueMarkup: () =>
          /* binding */ insertNewlineContinueMarkup,
      ...
      var dist_index_js_ = __webpack_require__(22819);
      ...
      var language_dist_index_js_ = __webpack_require__(4452);
      // EXTERNAL MODULE: ./node_modules/@codemirror/autocomplete/dist/index.js
      var dist = __webpack_require__(75128);
      ...
      var highlight_dist_index_js_ = __webpack_require__(45145); // CONCATENATED MODULE: ./node_modules/@lezer/markdown/dist/index.js
      class CompositeBlock {
        ...
        addChild(child, pos) {
          if (
            child.prop(common_dist_index_js_.NodeProp.contextHash) != this.hash
          )
            child = new common_dist_index_js_.Tree(
              child.type,
              child.children,
              child.positions,
              child.length,
        ...
        toTree(nodeSet, end = this.end) {
          ...
          return new common_dist_index_js_.Tree(
            ...
            makeTree: (children, positions, length) =>
              new common_dist_index_js_.Tree(
                common_dist_index_js_.NodeType.none,
                children,
                positions,
                length,
                this.hashProp
      ...
      class Line {
        ...
        forwardInner() {
          ...
          this.pos = newPos;
          this.next =
            newPos == this.text.length ? -1 : this.text.charCodeAt(newPos);
        }
      ...
      function skipForList(bl, cx, line) {
        ...
        return (
          ...
          (bl.type != Type.BulletList ||
            isHorizontalRule(line, cx, false) < 0) &&
        ...
        IndentedCode(cx, line) {
          ...
          while (cx.nextLine() && line.depth >= cx.stack.length) {
            if (line.pos == line.text.length) {
              ...
              to = cx.lineStart + line.text.length;
              let codeStart =
                cx.lineStart + line.findColumn(line.baseIndent + 4);
      ...
      class LinkReferenceParser {
        ...
        finish(cx, leaf) {
          if (
            (this.stage == 2 /* RefStage.Link */ ||
              this.stage == 3) /* RefStage.Title */ &&
        ...
        advance(content) {
          for (;;) {
            if (this.stage == -1 /* RefStage.Failed */) {
              ...
              if (
                !this.nextStage(
                  parseLinkLabel(content, this.pos, this.start, true)
                )
              )
                return -1;
      ...
      class BlockContext {
          ...
          if (start >= this.to) {
            ...
            if (this.ranges.length > 1) {
              ...
              while (this.ranges[rangeI].to < r.end) {
                ...
                r.text =
                  r.text.slice(0, this.ranges[rangeI - 1].to - textOffset) +
                  after;
        ...
        lineChunkAt(pos) {
          ...
          return pos + text.length > this.to
            ? text.slice(0, this.to - pos)
            : text;
        }
        ...
        addNode(block, from, to) {
          if (typeof block == 'number')
            block = new common_dist_index_js_.Tree(
        ...
        configure(spec) {
          ...
          if (nonEmpty(config.defineNodes)) {
            ...
            for (let s of config.defineNodes) {
              ...
              if (composite)
                skipContextMarkup[nodeTypes.length] = (bl, cx, line) =>
              ...
              let group = composite
                ...
                : !block
                  ...
                  : id >= Type.ATXHeading1 && id <= Type.SetextHeading2
                    ...
                    : ['Block', 'LeafBlock'];
              nodeTypes.push(
                common_dist_index_js_.NodeType.define({
                  id,
                  name,
                  props: group && [
                    [common_dist_index_js_.NodeProp.group, group]
                  ]
                })
              );
              if (style) {
            ...
            if (styles)
              nodeSet = nodeSet.extend(
                (0, highlight_dist_index_js_.styleTags)(styles)
              );
          }
        ...
        nodeTypes[i] = common_dist_index_js_.NodeType.define({
          ...
          props:
            i >= Type.Escape
              ...
              : [
                  [
                    common_dist_index_js_.NodeProp.group,
      ...
      const DefaultInline = {
        ...
        Entity(cx, next, start) {
          if (next != 38 /* '&' */) return -1;
          let m = /^(?:#\d+|#x[a-f\d]+|\w+);/i.exec(
            cx.slice(start + 1, start + 31)
          );
          return m
            ? cx.append(elt(Type.Entity, start, start + 1 + m[0].length))
            : -1;
        },
        ...
        HTMLTag(cx, next, start) {
          ...
          if (comment)
            return cx.append(
              elt(Type.Comment, start, start + 1 + comment[0].length)
            );
        ...
        Emphasis(cx, next, start) {
          ...
          let canOpen =
            leftFlanking && (next == 42 || !rightFlanking || pBefore);
          let canClose =
            rightFlanking && (next == 42 || !leftFlanking || pAfter);
          return cx.append(
            new InlineDelimiter(
              next == 95 ? EmphasisUnderscore : EmphasisAsterisk,
              start,
              pos,
              (canOpen ? 1 /* Mark.Open */ : 0) /* Mark.None */ |
        ...
        addDelimiter(type, from, to, open, close) {
          return this.append(
            new InlineDelimiter(
              ...
              from,
              to,
              (open ? 1 /* Mark.Open */ : 0) /* Mark.None */ |
                (close ? 2 /* Mark.Close */ : 0) /* Mark.None */
            )
          );
        }
          ...
          for (let i = from; i < this.parts.length; i++) {
            ...
            if (
              !(
                ...
                close.type.resolve &&
                close.side & 2 /* Mark.Close */
              )
            )
              continue;
            let emp =
              close.type == EmphasisUnderscore ||
            ...
            for (; j >= from; j--) {
              ...
              if (
                part instanceof InlineDelimiter &&
                part.side & 1 /* Mark.Open */ &&
                part.type == close.type &&
                // Ignore emphasis delimiters where the character count doesn't match
                !(
                  emp &&
                  (close.side & 1 /* Mark.Open */ ||
      ...
      class FragmentCursor {
        ...
        matches(hash) {
          ...
          return (
            tree &&
            tree.prop(common_dist_index_js_.NodeProp.contextHash) == hash
          );
        }
        takeNodes(cx) {
          let cur = this.cursor,
          ...
          for (;;) {
            ...
            if (cur.to - off <= cx.ranges[cx.rangeI].to) {
              ...
              let dummy = new common_dist_index_js_.Tree(
      ...
      const markdownHighlighting = (0, highlight_dist_index_js_.styleTags)({
        ...
        'ATXHeading1/... SetextHeading1/...':
          highlight_dist_index_js_.tags.heading1,
        'ATXHeading2/... SetextHeading2/...':
          highlight_dist_index_js_.tags.heading2,
        ...
        'HeaderMark HardBreak QuoteMark ListMark LinkMark EmphasisMark CodeMark':
          highlight_dist_index_js_.tags.processingInstruction,
      ...
      const parser = new MarkdownParser(
        new common_dist_index_js_.NodeSet(nodeTypes).extend(
      ...
      function parseCode(config) {
        ...
        let wrap = (0, common_dist_index_js_.parseMixed)((node, input) => {
          ...
          if (codeParser && (id == Type.CodeBlock || id == Type.FencedCode)) {
            ...
            htmlParser &&
            (id == Type.HTMLBlock || id == Type.HTMLTag)
          ) {
            return {
              parser: htmlParser,
        ...
        defineNodes: [
          {
            name: 'Strikethrough',
            style: {
              'Strikethrough/...': highlight_dist_index_js_.tags.strikethrough
            }
          },
          {
      ...
      const Table = {
        defineNodes: [
          ...
          {
            name: 'TableHeader',
            style: { 'TableHeader/...': highlight_dist_index_js_.tags.heading }
          },
          'TableRow',
        ...
        defineNodes: [
          {
            name: 'Task',
            block: true,
            style: highlight_dist_index_js_.tags.list
          },
      ...
      function autolinkURLEnd(text, from) {
        ...
        for (;;) {
          ...
          if (
            /[?!.,:*_~]/.test(last) ||
            (last == ')' &&
              count(text, from, end, ')') > count(text, from, end, '('))
          )
            end--;
          else if (
            last == ';' &&
      ...
      const Superscript = {
        defineNodes: [
          {
            name: 'Superscript',
            style: highlight_dist_index_js_.tags.special(
              highlight_dist_index_js_.tags.content
            )
          },
          {
      ...
      const Emoji = {
        defineNodes: [
          { name: 'Emoji', style: highlight_dist_index_js_.tags.character }
        ],
        parseInline: [
          {
            name: 'Emoji',
            parse(cx, next, pos) {
              ...
              if (
                ...
                return -1;
              return cx.addElement(
                cx.elt('Emoji', pos, pos + 1 + match[0].length)
              );
            }
          }
        ]
      ...
      var lang_html_dist = __webpack_require__(91962); // CONCATENATED MODULE: ./node_modules/@codemirror/lang-markdown/dist/index.js
      const data = /*@__PURE__*/ (0,
      language_dist_index_js_.defineLanguageFacet)({
      ...
      const headerIndent = /*@__PURE__*/ language_dist_index_js_.foldService.of(
        (state, start, end) => {
          for (
            let node = (0, language_dist_index_js_.syntaxTree)(
              state
            ).resolveInner(end, -1);
            node;
            node = node.parent
      ...
      function mkLang(parser) {
        return new language_dist_index_js_.Language(
          data,
          parser,
          [headerIndent],
          'markdown'
        ...
        return (info) => {
          if (info && languages) {
            ...
            if (typeof languages == 'function') found = languages(info);
              ...
              found =
                language_dist_index_js_.LanguageDescription.matchLanguageName(
                  languages,
                  info,
                  true
                );
            if (found instanceof language_dist_index_js_.LanguageDescription)
              return found.support
                ...
                : language_dist_index_js_.ParseContext.getSkippingParser(
                    found.load()
                  );
      ...
      class Context {
        ...
        blank(maxWidth, trailing = true) {
          ...
          if (maxWidth != null) {
            ...
            for (
              let i =
                this.to - this.from - result.length - this.spaceAfter.length;
              i > 0;
              i--
            )
              result += ' ';
      ...
      const insertNewlineContinueMarkup = ({ state, dispatch }) => {
        ...
        let dont = null,
          changes = state.changeByRange((range) => {
            if (
              !range.empty ||
              !markdownLanguage.isActiveAt(state, range.from, 0)
            )
              return (dont = { range });
            let pos = range.from,
              line = doc.lineAt(pos);
            ...
            if (inner.item && emptyLine) {
              ...
              if (
                ...
                (line.from > 0 &&
                  !/[^\s>]/.test(doc.lineAt(line.from - 1).text))
              ) {
                let next =
                  context.length > 1 ? context[context.length - 2] : null;
                let delTo,
                  insert = '';
                if (next && next.item) {
            ...
            if (nonTightList(inner.node, state.doc))
              insert =
                blankLine(context, state, line) + state.lineBreak + insert;
            changes.push({ from, to: pos, insert: state.lineBreak + insert });
            return {
              range: index_js_.EditorSelection.cursor(from + insert.length + 1),
              changes
            };
          });
          ...
          insert += context[i].blank(
            i < e
              ? (0, index_js_.countColumn)(
                  line.text,
                  4,
                  Math.min(line.text.length, context[i + 1].from)
                ) - insert.length
              : null,
            i < e
          );
                ...
                if (
                  ...
                  if (
                    ...
                    let insert = inner.blank(
                      (0, index_js_.countColumn)(line.text, 4, inner.to) -
                        (0, index_js_.countColumn)(line.text, 4, inner.from)
                    );
                    if (start == line.from)
                      insert = normalizeIndent(insert, state);
                    return {
                  ...
                  if (start < pos)
                    return {
                      range: index_js_.EditorSelection.cursor(start),
...
*/
      ...
      const htmlNoMatch = /*@__PURE__*/ (0, lang_html_dist.html)({
...
*/
      function markdown(config = {}) {
        ...
        let support = [htmlTagLanguage.support],
          defaultCode;
        if (
          defaultCodeLanguage instanceof language_dist_index_js_.LanguageSupport
        ) {
        ...
        if (addKeymap)
          support.push(
            index_js_.Prec.high(dist_index_js_.keymap.of(markdownKeymap))
          );
      ...
      function htmlTagCompletion(context) {
        ...
        if (!m) return null;
        let tree = (0, language_dist_index_js_.syntaxTree)(state).resolveInner(
          pos,
          -1
        );
      ...
      function htmlTagCompletions() {
        ...
        let result = (0, lang_html_dist.htmlCompletionSource)(
          new dist /* CompletionContext */._5(
            index_js_.EditorState.create({ extensions: htmlNoMatch }),
            0,
            true
          )
        );
...
//# sourceMappingURL=9311.69183e1a1bea0b23a91a.js.map?v=69183e1a1bea0b23a91a
