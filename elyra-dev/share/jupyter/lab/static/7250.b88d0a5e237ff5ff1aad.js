...
(self['webpackChunk_jupyterlab_application_top'] =
  ...
  [7250],
  {
    97250: (O, Q, P) => {
        ...
        if (g(O.next)) {
          ...
          if (O.next == o) {
            ...
            if (g(O.next)) {
              ...
              O.next == o ||
              O.next > 127 ||
              /\w/.test(String.fromCharCode(O.next))
            ) {
              return;
            }
          }
          if (O.next == s || O.next == S) {
            Q = true;
            O.advance();
            if (O.next == Z || O.next == _) O.advance();
            if (!n(O.next)) return;
            do {
              O.advance();
            } while (n(O.next));
          ...
          if (Q) O.acceptToken(r);
        } else if (O.next == t || O.next == Y) {
          if (O.next == t) O.advance();
          if (O.next != Y) return;
          O.advance();
          let Q = 0;
          while (O.next == p) {
      ...
      const V = new $.Lu((O) => {
        if (O.next == z) O.acceptToken(i, 1);
      });
      const T = new $.Lu((O) => {
        if (O.next == q) O.acceptToken(e, 1);
        else if (O.next == w) O.acceptToken(a, 1);
      });
      const y = (0, X.styleTags)({
        'const macro_rules struct union enum type fn impl trait let static':
          X.tags.definitionKeyword,
        'mod use crate': X.tags.moduleKeyword,
        'pub unsafe async mut extern default move': X.tags.modifier,
        'for if else loop while match continue break return await':
          X.tags.controlKeyword,
        'as in ref': X.tags.operatorKeyword,
        'where _ crate super dyn': X.tags.keyword,
        self: X.tags.self,
        String: X.tags.string,
        Char: X.tags.character,
        RawString: X.tags.special(X.tags.string),
        Boolean: X.tags.bool,
        Identifier: X.tags.variableName,
        'CallExpression/Identifier': X.tags.function(X.tags.variableName),
        BoundIdentifier: X.tags.definition(X.tags.variableName),
        'FunctionItem/BoundIdentifier': X.tags.function(
          X.tags.definition(X.tags.variableName)
        ),
        LoopLabel: X.tags.labelName,
        FieldIdentifier: X.tags.propertyName,
        'CallExpression/FieldExpression/FieldIdentifier': X.tags.function(
          X.tags.propertyName
        ),
        Lifetime: X.tags.special(X.tags.variableName),
        ScopeIdentifier: X.tags.namespace,
        TypeIdentifier: X.tags.typeName,
        ...
        'MacroInvocation/TypeIdentifier MacroInvocation/ScopedIdentifier/TypeIdentifier':
          X.tags.macroName,
        '"!"': X.tags.macroName,
        UpdateOp: X.tags.updateOperator,
        LineComment: X.tags.lineComment,
        BlockComment: X.tags.blockComment,
        Integer: X.tags.integer,
        Float: X.tags.float,
        ArithOp: X.tags.arithmeticOperator,
        LogicOp: X.tags.logicOperator,
        BitOp: X.tags.bitwiseOperator,
        CompareOp: X.tags.compareOperator,
        '=': X.tags.definitionOperator,
        '.. ... => ->': X.tags.punctuation,
        '( )': X.tags.paren,
        '[ ]': X.tags.squareBracket,
        '{ }': X.tags.brace,
        '. DerefOp': X.tags.derefOperator,
        '&': X.tags.operator,
        ', ; ::': X.tags.separator,
        'Attribute/...': X.tags.meta
      });
      const c = {
        __proto__: null,
        self: 28,
      ...
      const d = $.U1.deserialize({
        ...
        tokenizers: [V, T, R, 0, 1, 2, 3],
        topRules: { SourceFile: [0, 8] },
        specialized: [{ term: 281, get: (O) => c[O] || -1 }],
        tokenPrec: 15596
      });
      var f = P(4452);
        ...
        parser: d.configure({
          props: [
            f.indentNodeProp.add({
              IfExpression: (0, f.continuedIndent)({
                except: /^\s*({|else\b)/
              }),
              'String BlockComment': () => null,
              AttributeItem: (O) => O.continue(),
              'Statement MatchArm': (0, f.continuedIndent)()
            }),
            f.foldNodeProp.add((O) => {
              if (/(Block|edTokens|List)$/.test(O.name)) return f.foldInside;
              if (O.name == 'BlockComment')
                return (O) => ({ from: O.from + 2, to: O.to - 2 });
              return undefined;
            })
          ]
        }),
      ...
      function U() {
        return new f.LanguageSupport(l);
      }
    }
  }
]);
